...
while (min_pts<60):
    tps1 = time.time()
    model = cluster.HDBSCAN( min_cluster_size=min_pts)
    model.fit(datanp)
    tps2 = time.time()
   ...
   # plt.title("Données après clustering HDBSCAN (1) - MinPts= "+str(min_pts))
    #plt.show()
    ...
    if (min >d):
      min=d
    print("Time: ",round((tps2-tps1)*1000,2))
    print("CAH= ",metrics.calinski_harabasz_score(datanp,labels))
    print("DB= ",metrics.davies_bouldin_score(datanp,labels))
    print("SS= ",metrics.silhouette_score(datanp,labels))
